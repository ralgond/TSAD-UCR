...
from common import set_seed, UCRDataset, TestDataset, minmax_scale, create_window_list, augament, aggregate
from network_lukas import LukasAE, LukasEncoder

class Channel:
    def __init__(self, id, train_pos_list) -> None:
        self.id = id
        self.pos_samples = train_pos_list
    ...
    def pretrain(self):
        ...
        dataset = TestDataset(self.pos_samples)
    ...
    def train(self):
        ...
        print (f"==========>pos_samples.len: {len(self.pos_samples)}")
        ...
        for epoch in range(15):
            ...
            loss_epoch = 0.
            batch_epoch = 0
            for i, slices in enumerate(train_loader):
                optimizer.zero_grad()
    ...
    def init_center_c(self, train_loader: DataLoader, net: nn.Module, eps=0.01):
...
def main(file_no):
    ...
    channel = Channel(file_no, train_pos_list)
    channel.pretrain()
