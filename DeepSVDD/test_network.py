...
from network_lukas import LukasAE
...
if __name__ == "__main__":
    ...
    predict_data = []
    predict_loader = DataLoader(UCRDatasetForTest(scaled_all_data, 128), batch_size=100, shuffle=False)
    with torch.no_grad():
        for slice in predict_loader:
            # if len(slice) != 128:
            #     break
            print(slice.shape)
            ...
            print ("outputs.shape:", outputs.shape)
            predict_data.extend(outputs.squeeze().view(-1).cpu())
